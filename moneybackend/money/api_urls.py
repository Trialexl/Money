from django.urls import path, include
from . import catalog_views, dashboard_views, document_views, graphic_views, register_views, report_views, sync_views, technical_views, views
from rest_framework import routers


# API Router для всех ViewSets
router = routers.DefaultRouter()

# Справочники
router.register(r'cash-flow-items', catalog_views.CashFlowItemViewSet, basename='cash-flow-items')
router.register(r'wallets', catalog_views.WalletViewSet, basename='wallets')
router.register(r'projects', catalog_views.ProjectViewSet, basename='projects')
router.register(r'dashboard', dashboard_views.DashboardViewSet, basename='dashboard')
router.register(r'reports', report_views.ReportViewSet, basename='reports')
router.register(r'technical-health', technical_views.TechnicalHealthViewSet, basename='technical-health')

# Финансовые операции
router.register(r'receipts', document_views.ReceiptViewSet, basename='receipts')
router.register(r'expenditures', document_views.ExpenditureViewSet, basename='expenditures')
router.register(r'transfers', document_views.TransferViewSet, basename='transfers')
router.register(r'budgets', document_views.BudgetViewSet, basename='budgets')
router.register(r'auto-payments', document_views.AutoPaymentViewSet, basename='auto-payments')

# Регистры (только для чтения)
router.register(r'flow-of-funds', register_views.FlowOfFundsViewSet, basename='flow-of-funds')
router.register(r'budget-income', register_views.BudgetIncomeViewSet, basename='budget-income')
router.register(r'budget-expense', register_views.BudgetExpenseViewSet, basename='budget-expense')

# Графики планирования
router.register(r'expenditure-graphics', graphic_views.ExpenditureGraphicViewSet, basename='expenditure-graphics')
router.register(r'transfer-graphics', graphic_views.TransferGraphicViewSet, basename='transfer-graphics')
router.register(r'budget-graphics', graphic_views.BudgetGraphicViewSet, basename='budget-graphics')
router.register(r'auto-payment-graphics', graphic_views.AutoPaymentGraphicViewSet, basename='auto-payment-graphics')

app_name = 'money_api'

urlpatterns = [
    path('', include(router.urls)),
    path(
        'ai/execute/',
        views.AiAssistantViewSet.as_view({'post': 'execute'}),
        name='ai-execute',
    ),
    path(
        'ai/telegram-webhook/',
        views.AiAssistantViewSet.as_view({'post': 'telegram_webhook'}),
        name='ai-telegram-webhook',
    ),
    path(
        'ai/telegram-link-token/',
        views.AiAssistantViewSet.as_view({'post': 'telegram_link_token'}),
        name='ai-telegram-link-token',
    ),
    path(
        'onec-sync/outbox/',
        sync_views.OneCSyncOutboxViewSet.as_view({'get': 'list'}),
        name='onec-sync-outbox',
    ),
    path(
        'onec-sync/outbox/ack/',
        sync_views.OneCSyncOutboxViewSet.as_view({'post': 'ack'}),
        name='onec-sync-outbox-ack',
    ),
]
