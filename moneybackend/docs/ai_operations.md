...
`mode=agent` используется только веб-клиентом и не меняет Telegram pipeline. Перед
обычным разбором команды OpenRouter получает список доступных финансовых tools и
выбирает одно или несколько действий:

- создание денежных и инвестиционных операций
- остатки кошельков
- анализ расходов и бюджета
- обзор портфеля, позиции и ребалансировка
- справка по возможностям

Каждый выбранный tool преобразуется в самостоятельную команду и выполняется через
существующий `AiOperationService`. Поэтому matching справочников, уточнения,
подтверждение операций со скриншотов, защита от дублей и аудит остаются общими с
обычным web-режимом. Если OpenRouter недоступен или не выбрал tool, запрос безопасно
переходит в `classic` pipeline.
