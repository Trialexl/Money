...
- `wallet`
- `dry_run`
- `conversation` — включает диалоговые уточнения web-ассистента
- `mode` — `classic` (по умолчанию) или `agent`
- `history` — массив предыдущих `{role, content, image_index?}` для agent-режима
- `history_images` — до трех файлов прежних изображений в multipart; `image_index`
  связывает файл с исходным пользовательским сообщением (до 10 МБ на файл)

### Умный web-режим

`mode=agent` используется только веб-клиентом и не меняет Telegram pipeline.
OpenRouter напрямую получает текущий запрос, до 20 предыдущих сообщений диалога и
полные JSON Schema всех типизированных tools из `mcp_gateway.domain_tools`.

Agent loop выполняет до восьми шагов:

1. LLM отвечает самостоятельно или вызывает один или несколько tools.
2. Read-only tool выполняется от имени текущего Django-пользователя через внутренний API.
3. Результат tool возвращается LLM сообщением с ролью `tool`.
4. LLM может вызвать следующий tool или сформировать итоговый ответ.
5. Любой tool, меняющий данные, не выполняется сразу, а сохраняется в
   `AiPendingConfirmation` с `final_confirmation`.
6. После кнопки «Подтвердить» backend выполняет сохраненные вызовы и возвращает их
   результаты LLM для итогового ответа.

Аргументы write tools хранятся на сервере, поэтому клиент не может заменить их при
подтверждении. `/cancel` закрывает pending-команду без изменений. Если OpenRouter
недоступен или ключ отсутствует, API возвращает `503 agent_unavailable` с понятным
сообщением. Автоматического перехода в classic нет: тот может записывать текстовые
операции без подтверждения. Pending-команды двух режимов изолированы.

Внутренние tools проходят API permissions и middleware с `APP_DOMAIN` и HTTPS,
без сетевого запроса и без добавления `testserver` в `ALLOWED_HOSTS`.
Исполнитель по умолчанию запрещает любые записи; разрешение передается только
серверным обработчиком подтверждения. LLM должна искать существующие кошельки и
статьи через tools, а не спрашивать у пользователя UUID. На уточнениях веб-клиент
повторно передает до трех последних изображений из последних 20 сообщений,
поэтому прежний скриншот остается видимым модели.
...
- явная классификация пользователя имеет приоритет над банковскими иконками и подписями: «всё расходы» преобразует все распознанные денежные строки в расходы одного указанного кошелька, а «всё доходы» — в приходы; `wallet_from`/`wallet_to` в таком случае не запрашиваются
