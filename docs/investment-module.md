...
2. В investment module создается операция покупки инструмента в USD для расчета позиции, средней цены и P/L.
...
- `base_currency` в MVP всегда `USD`;
...
- хранит количество, цену, сумму в валюте операции и учетную сумму в USD;
...
- `cbr` - основной provider для cross-rate по `USD/EUR/RUB` через курсы ЦБ РФ;
...
`CbrFxRateProvider` поддерживает пары `USD/EUR/RUB`, включая `USD/RUB`, `USD/EUR`, `EUR/USD`, `RUB/USD`. Ошибки provider-а превращаются в `FxRateProviderError` и не должны влиять на обычный учет денег.
...
Позиции дополнительно возвращают `allocation_percent`, `target_allocation_percent`, `tolerance_percent`, `allocation_deviation_percent`, `allocation_deviation_usd` и `rebalance_amount_usd`.
...
Rebalance API возвращает текущую долю, целевую долю, отклонение в процентах и USD, а также расчетную сумму `buy/sell/hold`. Это аналитическая подсказка, не инвестиционная рекомендация и не команда на создание операций.
...
Экран портфеля хранит учет в USD, но позволяет переключить отображение в EUR/RUB. Переключатель использует последний `FxRateSnapshot` пары `USD -> выбранная валюта` и меняет только представление сумм.
